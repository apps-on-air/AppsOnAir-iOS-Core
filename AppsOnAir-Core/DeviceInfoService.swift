...
internal class DeviceInfoService {
    ...
    internal func getDeviceInfo(additionalInfo: [String: Any] = [:], completion: @escaping ([String: Any]) -> Void) {
        var appInfo: [String: Any] = [:]
        var deviceInfo: [String: Any] = [:]
        var systemInfo: [String: Any] = [:]
            ...
            if path.usesInterfaceType(.cellular) {
                DispatchQueue.main.asyncAfter(deadline: .now() + 0.01) {
                    ...
                    if #available(iOS 14.1, *),
                       tech == CTRadioAccessTechnologyNR || tech == CTRadioAccessTechnologyNRNSA {
                        networkType = "5G"
                    } else {
                        networkType = switch tech {
                        case CTRadioAccessTechnologyGPRS, CTRadioAccessTechnologyEdge: "2G"
